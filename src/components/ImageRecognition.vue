<template>
  <div>
      ...
      <div class="Appdisplay">
        <div class="edge">
          <div class="desktop">
            ...
            <div class="showplace">
              <img ref="img" class="myimg" src="../../static/image/hero@2x.jpg" alt="">
            </div>
            <span class="detail">{{detail}}</span>
            <p class="imgidentifyresults" v-if="isresult">识别结果：{{identifyresults}}</p>
          </div>
        </div>
    ...
    <el-button class="uploadgrade" v-if="isbtnshow" @click="goSpeechRecognition" type="success">提交所有分数</el-button>
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    goSpeechRecognition() {
      ...
      fetch(
        'http://120.79.193.101:8080/ApproximateComputing/imageRecognition/imageRecognitions',
        {
          method: 'post',
          headers: {
          ...
          body: JSON.stringify({
            imageRecognitions: this.alldata
          })
        }
      ).then(response => {
